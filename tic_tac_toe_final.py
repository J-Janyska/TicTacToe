...
def print_grid(grid: list) -> None:
    ...
    for i in range(0, 9, 3):
        print(f"| {grid[i]} | {grid[i+1]} | {grid[i+2]} |", line, sep="\n")
    print("\n")


def victory_test(grid: list, last_move_index: int, player: str) -> str:
    """
    Test occurrence of ['X', 'X', 'X'] or ['O', 'O', 'O']
    in triplets derived from the last_move_index.
    Return 'X' or 'O' if we have a winner, '' otherwise.
    """
...
def get_valid_move_number(grid: list) -> int:
    ...
    """
    while True:
        free_cells = [i for i, cell in enumerate(grid, 1) if cell == " "]
        print("Valid moves:", free_cells, "\n")
        inp = input("Your move? :")
        if inp.isnumeric() and int(inp) in free_cells:
            return int(inp) - 1
        else:
            print("This cell isn't free or you haven't entered a number 1-9 !")
...
def enumerated_input(prompt: str) -> str:
    ...
    while (user_input := input(prompt)) not in inputs_allowed:
        continue
    return user_input
